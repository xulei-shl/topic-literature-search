...
logger = logging.getLogger("wanfang_search.browser")


class BrowserManager:
    """负责 Camoufox 浏览器生命周期与会话持久化。"""

    def __init__(self, config: WanfangSearchConfig):
        self.config = config
    ...
    def start(self) -> Page:
        ...
        try:
            ...
            self._browser = NewBrowser(
                self._playwright,
                headless=WanfangSearchConfig.get_headless_mode(self.config.headless),
    ...
    def restore_session(self, target_url: Optional[str] = None) -> None:
        ...
        logger.debug("会话恢复: cookies数=%s, localStorage项=%s, 目标URL=%s",
                     len(self._context.cookies()) if cookies_loaded else 0,
                     "已加载" if storage_loaded else "无",
                     target_url)
