...
logger = logging.getLogger("vp_search.browser")


class BrowserManager:
    """负责 Camoufox 浏览器生命周期与会话持久化。"""

    def __init__(self, config: VpSearchConfig):
        self.config = config
    ...
    def start(self) -> Page:
        ...
        try:
            ...
            self._browser = NewBrowser(
                self._playwright,
                headless=VpSearchConfig.get_headless_mode(self.config.headless),
    ...
    def restore_session(self, target_url: Optional[str] = None) -> None:
        ...
        self._page.goto(self.config.home_url, timeout=self._navigation_timeout_ms())
        self._page.wait_for_load_state("domcontentloaded")
        storage_loaded = self.load_local_storage()
        if target_url and (storage_loaded or self._page.url != target_url):
            self._page.goto(target_url, timeout=self._navigation_timeout_ms())
            self._page.wait_for_load_state("domcontentloaded")
    ...
    def wait_for_manual_login(self) -> None:
        ...
        print("\n请在打开的 Camoufox 浏览器中完成维普登录。")
        print("登录完成后按回车继续保存会话。")
        input()
        time.sleep(self._navigation_settle_seconds())
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                for pid in result.stdout.strip().splitlines():
                    subprocess.run(["kill", "-9", pid], capture_output=True, timeout=self._subprocess_timeout_seconds())
                    cleaned += 1
        except Exception:
            pass
    ...
    def _navigation_settle_seconds(self) -> float:
        return float(self.config.navigation_timeout) / 30

    def _cleanup_settle_seconds(self) -> float:
        return float(self.config.action_timeout) / 60
