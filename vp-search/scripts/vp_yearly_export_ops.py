...
from vp_yearly_progress_store import YearlySearchProgressStore

logger = logging.getLogger("vp_search.yearly_export")


class VpYearlyExportMixin:
    """负责截至年份全量导出的逐年外层编排。"""

    YEARLY_MIN_YEAR = 1949
    YEARLY_VP_MIN_YEAR = 1989
    ...
    def _run_yearly_advanced_export(
        ...
        try:
            for year_index in range(next_year_index, len(tasks)):
                ...
                self._reset_page_for_next_year()
                ...
                if (year_index + 1) % self.ANTI_CRAWL_WAIT_INTERVAL == 0:
                    ...
                    new_page = self.browser_manager.start()
                    self.page = new_page
                    self.parser.page = new_page
                    self._ensure_yearly_search_page_ready()
        ...
        if yearly_result_files:
            final_file = output_dir / build_summary_output_filename(query=query, fallback="vp-yearly")
    ...
    def _build_yearly_export_tasks(
        ...
        if end_year < self.YEARLY_VP_MIN_YEAR:
            logger.error(
                f"date_to={date_to} 小于维普起始年份 {self.YEARLY_VP_MIN_YEAR}，无法执行逐年导出，已终止。"
            )
            raise SystemExit(1)
        normalized_years = sorted(
            {
                ...
                if str(year_text).strip().isdigit() and int(str(year_text).strip()) >= self.YEARLY_VP_MIN_YEAR
            },
            key=int,
        )
        if not normalized_years:
            return []
        effective_start = int(date_from) if date_from else self.YEARLY_VP_MIN_YEAR
        if effective_start < self.YEARLY_VP_MIN_YEAR:
            effective_start = self.YEARLY_VP_MIN_YEAR
        first_year_task = None
        ...
        for year_text in normalized_years:
            ...
            if first_year_task is None:
                date_from_value = "" if not date_from or int(date_from) < self.YEARLY_VP_MIN_YEAR else date_from
                first_year_task = {
                    "query": query,
                    "year": task_year,
    ...
    def _reset_page_for_next_year(self) -> None:
        """每个年份执行完成后刷新到高级检索页，确保下一年干净入口。"""
        try:
            ...
            pass
        try:
            if self.page.url and "qikan.cqvip.com" in self.page.url:
                self.page.goto(self.advanced_search_url, timeout=self._navigation_timeout_ms())
    ...
    def _write_empty_year_result(self, output_dir: Path, task: dict[str, Any], result: dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "状态: no_results",
            f"年份: {task['year']}",
            f"日期范围: {task['date_from']} ~ {task['date_to']}",
    ...
    def _rerun_years_until_export_consistent(
        ...
        rerun_completed_years: list[str] = []
        while True:
            if saved_pending_years:
                failure_years_set = set(item["year"] for item in failures)
                pending_set = set(saved_pending_years)
                completed_in_batch = list(pending_set & failure_years_set & set(rerun_completed_years))
                for year in completed_in_batch:
                    saved_pending_years.remove(year)
                rerun_years = [y for y in saved_pending_years if y not in rerun_completed_years]
                if rerun_years:
                    logger.info("从已保存的重跑列表继续: %s", ", ".join(rerun_years))
                    pending_task_map = {task["year"]: task for task in tasks}
                    failures = []
                    for year in rerun_years:
                        if year in pending_task_map:
                            task = pending_task_map[year]
                            year_output_dir = output_dir / f"year-{year}"
                            report_path = find_latest_yearly_summary_report(year_output_dir)
                            merged_excel_path = find_latest_yearly_merged_excel(year_output_dir)
                            if report_path and merged_excel_path:
                                try:
                                    reported_total = read_report_total(report_path)
                                    actual_rows = count_excel_rows(merged_excel_path)
            ...
            for failure in failures:
                ...
                sub_progress_file = self._build_yearly_sub_progress_file(output_dir, task)
                self._cleanup_year_output_dir(output_dir, task)
                self._save_yearly_progress_snapshot(
                    progress_store=progress_store,
                    status="running",
                ...
                rerun_completed_years.append(task["year"])
    ...
    def _write_yearly_summary_report(
        ...
        report_path = output_dir / build_summary_report_output_filename(query=query, fallback="vp-yearly")
        lines = [
            f"检索词: {query}",
            f"状态: {status}",
            "模式: yearly",
