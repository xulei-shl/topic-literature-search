...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="vp-search",
        description="基于 Camoufox 的维普检索工具",
    )
    ...
    advanced_parser.add_argument(
        "-n", "--max-download", "--max-export", dest="max_download", type=int, default=None, help="最多导出条数"
    )
    advanced_parser.add_argument("--progress-file", type=Path, default=None, help="断点续跑进度文件路径")
    return parser


def build_config(args: argparse.Namespace) -> VpSearchConfig:
    """构造配置。"""
...
def _run_command_once(args: argparse.Namespace, config: VpSearchConfig) -> dict:
    ...
    try:
        ...
        interactor = VpSearchInteractor(page, config, browser_manager)
