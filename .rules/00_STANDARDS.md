...
**所有核心流程必须留痕。** 涉及核心功能开发、业务逻辑变更、Bug 修复、测试策略调整、接口/输出行为变化或正式 Code Review 结论时，必须在 `docs/` 下生成对应的 Markdown 文件。

以下场景可免留痕：
- 纯文案修订、错别字修复、排版整理。
- 无行为变化的 README、注释、路径说明、规则措辞微调。
- 范围极小且不影响实现逻辑、测试策略、接口输出的配置说明更新。

如无法快速判断是否属于“微小改动”，默认要求留痕。严禁对需要留痕的代码或流程改动省略文档更新。

## 2. 代码哲学 (Code Philosophy)
- **KISS原则**: 简洁至上，拒绝过度设计。
- **DRY原则**: 拒绝重复代码，积极模块化。
- **单一职责**: 一个函数只做一件事。
- **文件长度**: 单个文件建议控制在 500 行以内。
